...
def googlenet(pretrained=False, progress=True, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = model_zoo.load_url(model_urls["googlenet"], progress=progress)
...
class GoogLeNet(nn.Module):
    ...
    def _forward(self, x):
        ...
        # N x 1024
        x = self.dropout(x)
        # x = self.fc(x)  # Remove the last fc layer
...
class Inception(nn.Module):
    ...
    def __init__(self, in_channels, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj, conv_block=None):
        ...
        self.branch4 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1, ceil_mode=True), conv_block(in_channels, pool_proj, kernel_size=1)
        )
