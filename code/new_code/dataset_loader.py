...
class BreastDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, json_path, data_dir_path='C:\datasets\paper_patches', clinical_data_path=None, is_preloading=True):
        ...
        Args:
            json_path (str): path to (updated_test/train/val-type-0.json)
            data_dir_path (str, optional): path where patches is stored
            clinical_data_path (str, optional): path to processed excel file containing clinical information.
            is_preloading (bool, optional): preloads WSI images
    ...
    def __getitem__(self, index):
        metastasis_label = int(self.json_data[index]["metastasis_label"])
        status_label = int(self.json_data[index]["status_label"])
        ...
        data["metastasis_label"] = metastasis_label
